...
const removeCookie = (cookieName: string) => {
    let name = cookieName.startsWith(cookiePrf) ? cookieName : `${cookiePrf}${cookieName}`;
...
const replaceCookie = (cookieName: string, cookieValue: string, expireTime: Date | null) => {
    const date = new Date();
    ...
    /**create cookie */
    document.cookie = `${cookiePrf}${cookieName}=${cookieValue};expires=${expireTime ? expireTime: date}; domain=${environment.baseDomain}; path=/`;
}
